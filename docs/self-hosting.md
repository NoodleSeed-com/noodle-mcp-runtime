...
Required-mode session responses additionally advertise `executionAdmission: 'required'`. The browser SDK and the served `embed.js` prepare and execute a durable message operation only when this capability is present. Endpoint presence alone does not opt a legacy session into protected execution. Preparation failure never falls back to an unkeyed turn. A lost preparation response retains the original key and turn snapshot; an explicit retry recovers that preparation. A lost or truncated execution checks status before any retry and only delivers the same operation if it is still prepared. Completed, executing or unknown work is never automatically replayed. Resetting the conversation is an explicit action. Required execution currently does not support resume or confirmation continuation outside this operation contract.

Public website deployments return a stable non-secret `embedId`. Paste `<script src="https://CORE_ORIGIN/v1/assistant/embed.js" data-embed-id="pub_..." async></script>` on an allowed origin. The script derives the Core service origin from its URL. `GET /v1/orgs/:org/apps/:app/envs/:env/assistant/embeds` recovers the current embed IDs and surface view. Public sessions select the highest active version in that environment; separate preview and public environments when their release schedules differ.

The configured admission gate also runs before public session minting consumes capacity or creates a session. It receives `method: 'assistant/public-sessions'`, `category: 'protocol'`, the resolved deployment/version and `assistantSurface: {kind:'public', origin, publicEmbedId}`. Origin and embed ID come from the validated runtime surface, and later assistant admissions use persisted session provenance. Anonymous session IDs never become verified `subject` values. Policy changes can therefore deny old sessions as well as new mints. Existing technical public capacity limits remain separate from external business policy.
